...
class SimAnalysis(QWidget):
   ...
   - hexagonal SIM (1 angle, 7 phases).
        ...
        if self.is_image_in_layers():
            ...
            if self.showXcorr.val and hasattr(self, 'h'):
                ...
                if self.proc.current_data == Accel.USE_TORCH.value:
                    ixf = np.squeeze(
                        self.h.crossCorrelations_pytorch(im))  # TODO check if traspose or flip should be added
        ...
        if self.is_image_in_layers():
            ...
            if self.showCarrier.val and self.isCalibrated:
                ...
                for idx, (kx, ky) in enumerate(zip(kxs, kys)):
                    pc[idx, 0] = ky / dky + Ny / 2
                    pc[idx, 1] = kx / dkx + Nx / 2
    ...
    def single_plane_reconstruction(self):
        ...
        if self.proc.current_data == Accel.USE_TORCH.value:
            imageSIM = self.h.reconstruct_pytorch(
                rdata.astype(np.float32))  # TODO:this is left after conversion from torch
        elif self.proc.current_data == Accel.USE_CUPY.value:
            imageSIM = self.h.reconstruct_cupy(
                rdata.astype(np.float32))  # TODO:this is left after conversion from torch
        else:
    ...
    def stack_reconstruction(self):
        ...
        def _stack_reconstruction():
            ...
            for zidx in range(sz):
                ...
                if self.proc.current_data == Accel.USE_TORCH.value:
                    stackSIM[zidx, :, :] = self.h.reconstruct_pytorch(
                        phases_stack.astype(np.float32))  # TODO:this is left after conversion from torch
                elif self.proc.current_data == Accel.USE_CUPY.value:
                    stackSIM[zidx, :, :] = self.h.reconstruct_cupy(
                        phases_stack.astype(np.float32))  # TODO:this is left after conversion from torch
                else:
